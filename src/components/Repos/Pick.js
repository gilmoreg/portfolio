...
const Pick = props => (
  <section className="Pick">
    ...
    <p className={props.more ? '' : 'hidden'}>One interesting challenge I found was in implementing <strong>rate limiting</strong>. The package I chose was only applying limiting at random to my great frustration; after stepping through the source I learned that the regex test for which path to limit was failing at random. It was then that I learned what I shall never forget: that the <strong>Javascript RegExp object</strong> is stateful when the global flag is enabled. Thus the result were different depending on whether another test was being run at the same time. Another painful lesson learned!</p>
